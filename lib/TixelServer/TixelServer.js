...
function TixelServer (config) {
    this.port = (type(config.port) === 'number') ? config.port : 80;
    ...
    var self = this;
    this.events.on('input.updateState', function () {
...
TixelServer.prototype.triggerInputEvents = function () {
    if (type(this.touchBuffer) === 'array') {
        touchEvents.update(this.touchBuffer);
    }
};
