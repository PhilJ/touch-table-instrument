...
var TixelSelection = require('./TixelSelection.js'),
    ...
    addColors = require('./utils.js').addColors,
 ...
 * @property {ndarray} canvas        Canvas were child pixels are rendered on
...
function TixelElement (config) {
    ...
    this.valueBuffer =  ndarray(new Array(this.size[0] * this.size[1]), this.size);
    ops.assigns(this.valueBuffer, null);
    this.opacity = ndarray(new Array(this.size[0] * this.size[1]), this.size);
    ops.assigns(this.opacity, 1);

    // Initialize output (pixel matrix) with Array() and defaultColor as default value
    this.canvas = ndarray(new Array(this.size[0] * this.size[1]), this.size);
    ops.assigns(this.canvas, this.defaultColor);
    this.output = ndarray(new Array(this.size[0] * this.size[1]), this.size);
    ops.assigns(this.output, this.defaultColor);

    this.outputBuffer = ndarray(new Array(this.size[0] * this.size[1]), this.size);
    ops.assigns(this.outputBuffer, null);
...
TixelElement.prototype.render = function (type) {
    this.output = this.getCanvasDuplicate();
    
    this._renderValues();
    this._renderChildElements();

    this._applyOutputBuffer();
    this._applyValueBuffer();
    ...
    if (type === 'ndarray') {
        ...
        return unpack(this.output);
    }
};

TixelElement.prototype.getCanvasDuplicate = function () {

    var output = ndarray(new Array(this.size[0] * this.size[1]), this.size);
    var self = this;
    iterate(output, function (x, y) {
        var canvasColor = self.canvas.get(x,y);
        output.set(x, y, canvasColor);
    });
    return output;
};
...
TixelElement.prototype._renderChildElements = function () {
    for (var c in this.childElements) {
        var childTixel = this.childElements[c];
        var childRenderResult = childTixel.render('ndarray');
        this._applyChildMatrix(childRenderResult, childTixel.opacity, childTixel.origin);
    }
};
 ...
 */
TixelElement.prototype._applyChildMatrix = function (colorMatrix, opacityMatrix, position) {
    var self = this;
    iterate(colorMatrix, function (x,y) {
        var childColor = colorMatrix.get(x,y),
            opacity    = opacityMatrix.get(x,y),
            px         = x + position[0],
            py         = y + position[1];

        if (self.isValidPosition([px,py]) === true) {
...
/**
 * Is valid position on matrix
 *
 * @param {array}    position Position to validate
 * @return {boolean} true, if valid position
 */
TixelElement.prototype.isValidPosition = function (position) {
    if (position[0] >= 0 && position[0] < this.size[0] && 
        position[1] >= 0 && position[1] < this.size[1]) {
        return true;
    }
    return false;
}
...
TixelElement.prototype.forEachPixel = function (callback, addData) {
    ...
    var self = this;
    iterate(this.canvas, function (x,y) {
        if (addData === true) {
