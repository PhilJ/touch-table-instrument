...
function TixelElement (config) {
    ...
    this.defaultColor = (typeof config.defaultColor === 'string') ? config.defaultColor : '000000';
    ...
    this.colorMap = Array.isArray(config.colorMap) ? config.colorMap : [this.defaultColor];
...
TixelElement.prototype.getMappedColor = function (value) {
    var noOfColors = this.colorMap.length;
    if (noOfColors > 0) {
        // Calculate index in colorMap
        var colorIndex = Math.floor( noOfColors * value);

        // Fix values which are to large or small (e.g. because value is out of range)
        if (colorIndex < 0) colorIndex = 0;
        if (colorIndex >= noOfColors) colorIndex = noOfColors - 1;

        if (typeof this.colorMap[colorIndex] === 'string') {
...
TixelElement.prototype.setSelectedPixels = function (color) {
    ...
    if (Array.isArray(color)) {
        var self = this;
        this.forEachSelectedPixel(function (position) {
            self.canvas.setBuffer(position[0], position[1], color);
        });
    }
    return this;
};
...
TixelElement.prototype.setSelectedValues = function (value) {
    ...
    if ( Array.isArray(value) ) {
        var self = this;
        this.forEachSelectedPixel(function (position) {
            self.valueCanvas.setBuffer(position[0], position[1], value);
        });
    }     
    return this; 
};
...
TixelElement.prototype.forEachPixel = function (callback, addData) {
    var addData = (typeof addData === 'boolean') ? addData : false;
    var self = this;
