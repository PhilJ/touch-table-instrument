 ...
 */
TixelMatrix.prototype.setBuffer = function (position, buffer) {
	if (utils.isPosition(position) === true && utils.isOnMatrix(position, this.size) === true && type(buffer) === 'array') {
		this.buffer[ position[0] ][ position[1] ] = buffer;
	}
};
...
TixelMatrix.prototype.applyBuffer = function () {
	...
	this.forEach(function (x, y) {
        ...
        if (type(buffer) === 'array' && buffer.length > 0) {
